...
Number *Complex::add(Number * num) {
    ...
    if (num-> types() == INTEGER) {
        num = dynamic_cast<INTEGER*> (num);
    }
    else if (num->types() == FLOAT) {
        num = dynamic_cast<FLOAT*> (num);
    }
    else if (num -> types() == COMPLEX) {
        num = dynamic_cast<COMPLEX*> (num):
    }
    this->set_val(this->real() + num->real(), this->imag() + num->imag());
    delete(num);
    if (this->types() == COMPLEX) {
...
Number *Complex::mul(Number * num) {
    ...
    this->set_val((this->real() * num->real()) - (this->imag() * this->imag()) , this->real() * num->imag() + this->imag * this>real());
    delete(num);
    if (this->types() == COMPLEX) {
        return dynamic_cast<COMPLEX*> this;
    }
